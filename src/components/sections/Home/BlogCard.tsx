export default function BlogCard({ image, tag, title, date, views, href }) {
    return (
        <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow transition flex flex-col overflow-hidden h-[320px] border border-gray-100">
            <img src={image} alt={title} className="w-full h-[200px] object-cover rounded-2xl" />
            <div className="p-5 flex flex-col flex-1">
                <span className="text-[#16c79a] text-[15px] font-semibold mb-1">{tag}</span>
                <div className="flex flex-row items-center gap-2 w-full justify-between">
                    <div className="font-bold text-base text-gray-900 mb-2 leading-snug line-clamp-2 w-5/6">{title}</div>
                    <a
                        href={href}
